...
class ModuleEventlist extends EventsExt
{
    ...
    protected function compile(): void
    {
        ...
        if ('' !== $this->cal_format_ext) {
            ...
            } elseif (2 === \count($times)) {
                $strBegin = strtotime($times[0]) ?: time();
        ...
        // Get all events
        $arrAllEvents = $this->getAllEventsExt($this->cal_calendar, $strBegin, $strEnd, [$this->cal_holiday, $showRecurrences]);
            ...
            foreach ($days as $day => $events) {
                foreach ($events as $event) {
                    ...
                    if (class_exists('leads\leads') && $event['useRegistration']) {
                        if ($event['regperson']) {
                            $values = StringUtil::deserialize($event['regperson']);
...
//                    $event['datetime'] = date('Y-m-d', $day);
                    ...
                    if (1 === (int) $event['addTime'] && 1 === (int) $event['ignoreEndTime']) {
...
//                        }
                    ...
                    if ($event['recurring']) {
                        $arrRepeat = StringUtil::deserialize($event['repeatEach']) ?: null;
                        $unit = $arrRepeat['unit'];
                    }

                    if ($event['recurringExt']) {
                        $arrRepeat = StringUtil::deserialize($event['repeatEachExt']) ?: null;
                        $unit = $arrRepeat['unit'];
                    }
        ...
        for ($i = $offset; $i < $limit; ++$i) {
            ...
            if ($strDate !== $event['firstDate']) {
                ...
                $objTemplate->classHeader = (0 === $dayCount % 2 ? ' even' : ' odd').(0 === $dayCount ? ' first' : '').($event['firstDate'] === $arrEvents[$limit - 1]['firstDate'] ? ' last' : '');
            ...
            $objTemplate->classList = $event['class'].(0 === $headerCount % 2 ? ' even' : ' odd').(0 === $headerCount ? ' first' : '').($blnIsLastEvent ? ' last' : '').' cal_'.$event['parent'];
            $objTemplate->classUpcoming = $event['class'].(0 === $eventCount % 2 ? ' even' : ' odd').(0 === $eventCount ? ' first' : '').(($offset + $eventCount + 1) >= $limit ? ' last' : '').' cal_'.$event['parent'];
