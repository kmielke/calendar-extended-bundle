...
$GLOBALS['TL_DCA']['tl_calendar_events']['fields']['repeatFixedDates'] = [
    ...
    'eval' => [
        'columnsCallback' => ['tl_calendar_events_ext', 'listFixedDates'],
...
$GLOBALS['TL_DCA']['tl_calendar_events']['fields']['useExceptions'] = [
    ...
    'sql' => "char(1) NOT NULL default ''",
    'save_callback' => [
        ['tl_calendar_events_ext', 'checkExceptions'],
    ],
];
...
$GLOBALS['TL_DCA']['tl_calendar_events']['fields']['recurring'] = [
    ...
    'sql' => "char(1) NOT NULL default ''",
    'save_callback' => [
        ['tl_calendar_events_ext', 'checkRecurring'],
    ],
];
...
$GLOBALS['TL_DCA']['tl_calendar_events']['fields']['regperson'] = [
    ...
    'filter' => false,
    'inputType' => 'multiColumnWizard',
    'load_callback' => [['tl_calendar_events_ext', 'getmaxperson']],
    'eval' => [
        'tl_class' => 'w50 clr',
        'columnsCallback' => ['tl_calendar_events_ext', 'setmaxperson'],
...
$GLOBALS['TL_DCA']['tl_calendar_events']['fields']['regform'] = [
    ...
    'filter' => true,
    'inputType' => 'select',
    'options_callback' => ['tl_calendar_events_ext', 'listRegForms'],
...
$GLOBALS['TL_DCA']['tl_calendar_events']['fields']['repeatExceptions'] = [
    ...
    'inputType' => 'multiColumnWizard',
    'eval' => [
        'columnsCallback' => ['tl_calendar_events_ext', 'listMultiExceptions'],
        'buttons' => ['up' => false, 'down' => false],
    ],
    'sql' => 'blob NULL',
];
...
class tl_calendar_events_ext extends Backend
{
    ...
    public function adjustTime(DataContainer $dc)
    {
        ...
        if ($dc->activeRecord->recurring) {
            ...
            while ($next <= $end) {
                ...
                $wdays = is_array(StringUtil::deserialize($dc->activeRecord->repeatWeekday))
                    ? StringUtil::deserialize($dc->activeRecord->repeatWeekday)
                    : false;

                if ('days' === $unit && 1 === $value && $wdays) {
                    $wday = date('N', $next);
                    $store = in_array($wday, $wdays, true);
                }
    ...
    {
        $values = StringUtil::deserialize($var);
